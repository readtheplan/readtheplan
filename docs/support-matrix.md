...
| Puppet project / Server policy / runtime / deployment / Bolt | `Puppetfile`, module metadata, Hiera, `puppet.conf`, `environment.conf`, `puppetdb.conf`, Puppet Server `auth.conf`/`ca.conf`/`webserver.conf`/`puppetserver.conf`/`web-routes.conf`, r10k, Bolt project/inventory, `plans/*.yaml`, `tasks/*.json`, or shell/PowerShell/Python/Ruby task programs | `readtheplan puppet-project modules/site/tasks/deploy.sh` | Forge/Git dependencies, environment code/cache/commands, PuppetDB transport/fail-open/quorum, strict HOCON authorization and API scope, mTLS/TLS, CA signing, JRuby paths/environment/lifecycle, routes, r10k deployment, and Bolt commands/tasks/scripts/nested plans/transfers/resources; bounded non-executing task-code inspection covers dynamic/process execution, destructive changes, network access, privilege, filesystem/system mutation, unsafe deserialization, parameters/secrets, no-op/remote contracts, and runtime trust boundaries | Built-in |
