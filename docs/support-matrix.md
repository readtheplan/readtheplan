...
| Ansible project / inventory / execution environment | `ansible.cfg`, Galaxy requirements, static/dynamic inventory, `execution-environment.yml`, or `ansible-navigator.yml`/JSON | `readtheplan ansible-project execution-environment.yml` | Controller transport/privilege, plugins/callbacks, Galaxy dependencies, inventory scope/identity/trust, Builder image/dependency/build provenance, Navigator isolation/privilege/mounts/environment/pull policy/commands, credentials, and non-execution boundaries | Built-in |
