...
| Ansible project / inventory / Controller export / content policy / execution environment / Molecule | `ansible.cfg`, requirements, inventory, Automation Controller/AWX export JSON/YAML, `galaxy.yml`, role/collection metadata, Ansible-lint config, `execution-environment.yml`, Navigator, or Molecule YAML | `readtheplan ansible-project controller-export.json` | Controller/AWX import assets, RBAC, credentials, projects, inventories/sources, job/workflow templates, schedules, notifications, execution environments, controller trust/privilege, dependency provenance, inventory scope, collection/role metadata, lint policy, Builder/Navigator isolation, and Molecule providers/sequences/cleanup | Built-in |
...
| Chef project / dependencies / runtime | `Policyfile.rb`, `Policyfile.lock.json`, `Berksfile`, modern/legacy `Berksfile.lock`, `metadata.rb`, runtime configs, supported `.d` fragments, or Test Kitchen YAML | `readtheplan chef-project .kitchen.yml` | Policy/run lists, cookbook provenance, Berkshelf migration/source ordering/groups/solver, direct and transitive lock graph integrity, Test Kitchen layered drivers/provisioners/transports/verifiers, lifecycle hooks, privilege/host access, credentials, mutable toolchains, client identity/convergence, Workstation bootstrap/SSH, Solo remote content, Server TLS/LDAP/database controls, duplicate overrides, and dynamic Ruby/ERB/external-state boundaries | Built-in |
...
- Ansible configuration/variable precedence, combined inventory sources, inventory-plugin code
  ...
  pruning, Molecule base-config merges, environment interpolation,
