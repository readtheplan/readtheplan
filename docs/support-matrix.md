...
| Puppet project / Server policy / runtime / deployment / Bolt / Ruby extensions | `Puppetfile`, module metadata, Hiera, `puppet.conf`, `environment.conf`, `puppetdb.conf`, Puppet Server `auth.conf`/`ca.conf`/`webserver.conf`/`puppetserver.conf`/`web-routes.conf`, r10k, Bolt project/inventory, plans/tasks, or module Ruby under `lib/facter`, `lib/puppet/functions`, `lib/puppet/parser/functions`, `lib/puppet/type`, `lib/puppet/provider`, and `lib/puppet/reports` | `readtheplan puppet-project modules/site/lib/facter/cloud_inventory.rb` | Forge/Git dependencies, environment/server/PuppetDB/r10k policy, Bolt automation, and bounded non-executing custom-fact, modern/legacy function, type, provider, and report-processor inspection; surfaces dynamic/process execution, filesystem/network mutation, unsafe deserialization, compiler/catalog access, sensitive data, plugin loading, and explicit server/agent/report runtime boundaries | Built-in |
...
- Ansible configuration/variable precedence, combined inventory sources, inventory-plugin live API
  ...
  installed plugin/import loader precedence, unrecognized module languages/binaries, runtime
