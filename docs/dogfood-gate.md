...
The gate is deliberately an aggregate non-regression policy. Existing findings
remain visible in the uploaded JSON evidence, while
...
This detects net increases at a risk level, parser failures, and scan-coverage
reductions. It does not fingerprint individual findings, so a new finding can be
offset by another finding disappearing at the same risk level. A change that
intentionally alters the inventory or classifications must include the
corresponding baseline change for explicit review. Never regenerate or raise the
baseline automatically in CI. Prefer fixing findings and lowering its ceilings.
