...
const setupHelpLink = document.querySelector("#setupHelpLink");
...
// Optional free community help inbox. Keep configurable and visible.
const SETUP_HELP_EMAIL = "info@readtheplan.dev";
...
const repoProfileLabels = {
  ...
  regulated: "20+ repos / regulated estate",
};
...
function generateChecklist(state) {
  const checks = [
    `Setup profile: ${state.organization}; ${repoProfileLabels[state.repoProfile]}; ${maturityLabels[state.maturity]}.`,
  ...
  if (state.adoptionBlocker !== "not provided") {
    checks.push(`First adoption risk to resolve: ${state.adoptionBlocker}.`);
  }

  checks.push("Run the first free setup on one private repo, then repeat the same gate across the remaining repo profile.");
...
function generateSummaryRows(state) {
  return [
    ...
    },
    {
      ...
      behavior: `Checklist and optional setup-help handoff stay browser-local for ${state.organization}.`,
    },
  ];
}
...
function updateSetupHelpLink(state) {
  const subject = encodeURIComponent(`readtheplan free setup help: ${state.organization}`);
  const body = encodeURIComponent(
    [
      "readtheplan free setup help",
      "",
      "Client context:",
  ...
  setupHelpLink.href = `mailto:${SETUP_HELP_EMAIL}?subject=${subject}&body=${body}`;
  handoffNote.textContent = `Optional free setup-help email opens a mailto draft to ${SETUP_HELP_EMAIL}; no form submission or plan attachment is created.`;
}
