...
| Ansible | `readtheplan ansible roles/web/tasks/main.yml` | Strict playbook, reusable role-task, and handler YAML analysis covering blocks, handler scope/notifications, imports/includes, privilege, delegation, check/error controls, dynamic expressions, secret environments, identity, and supply-chain semantics |
| Ansible project / inventory / Controller export / Event-Driven Ansible / content policy / execution environment / Molecule | `readtheplan ansible-project extensions/eda/rulebooks/remediate.yml` | Automation Controller/AWX import-export assets plus Event-Driven Ansible rulebooks covering source/filter plugins, webhook ingress, conditions, playbook/module and Controller template actions, concurrency, retries, event propagation, secrets and activation boundaries; also controller transport, Galaxy dependencies, inventory, collection/role metadata, Ansible-lint policy, Builder/Navigator isolation, and Molecule providers/sequences/verifiers |
...
| Jenkins project / Shared Library | `readtheplan jenkins-project vars/deploy.groovy` | Plugin Installation Manager catalogs, Jenkins Job Builder YAML/JSON, and Shared Library `vars/*.groovy` / `src/**/*.groovy`; controller APIs, dependency loading, commands, credentials, mutable globals, CPS/serialization, resources, filesystem/network access, and trust/runtime boundaries |
...
| Chef cookbook content | `readtheplan chef cookbooks/base/resources/application.rb` | Recipes, attributes, custom resources, libraries, legacy providers/definitions, and ERB templates; resources/actions, precedence, sensitive properties/interpolation, arbitrary Ruby commands, filesystem/network access, Chef extensions, external data, guards, notifications, permissions, and includes |
...
| Puppet project / Server policy / runtime / deployment / Bolt | `readtheplan puppet-project modules/site/plans/deploy.yaml` | Forge/Git modules, metadata, Hiera, agent settings, per-environment code/cache policy, PuppetDB failover and persistence, Puppet Server authorization/mTLS/CA/JRuby/routes, r10k/Code Manager deployment, and Bolt project/inventory, YAML plans, task metadata, commands, scripts, nested plans, transfers, resources, privilege, failure handling, sensitive inputs, no-op/remote execution, implementation selection, and dynamic-expression boundaries |
...
- Ansible playbook, reusable task-file, and handler analysis never executes Ansible, renders Jinja, evaluates conditions/lookups, expands imported or included content, loads role/collection plugins, or resolves inventory and Vault values; caller hosts, inherited variables/tags/privilege, module search paths, embedded custom code, handler insertion/collisions/notification routing, CLI selection, and live task results remain review boundaries
...
- Bolt project, inventory, YAML plan, and task-metadata analysis is static and never runs Bolt, evaluates Puppet expressions, resolves plugins, executes tasks/commands/scripts/nested plans, transfers files, applies resources, or reads sibling implementation code; configuration precedence, installed module/plugin provenance, implementation contents, resolved dynamic inventory, external secrets, target facts/variables/state, transport behavior, plan cache, and live results remain runtime boundaries
