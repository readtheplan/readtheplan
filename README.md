...
| Ansible project / inventory / content policy / execution environment / Molecule | `readtheplan ansible-project galaxy.yml` | Controller transport, privilege, plugins/callbacks, Galaxy dependencies, static/dynamic inventory, collection and role metadata, runtime redirects/action groups, role argument specs, Ansible-lint policy bypasses, Builder/Navigator isolation, and Molecule drivers/platforms/provider settings/sequences/verifiers, with credential, provenance, cleanup, and execution boundaries |
