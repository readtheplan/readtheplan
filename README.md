...
| Puppet project / Server policy / runtime / deployment / Bolt / Ruby extensions | `readtheplan puppet-project modules/site/lib/facter/cloud_inventory.rb` | Forge/Git modules, metadata, Hiera, agent settings, per-environment code/cache policy, PuppetDB failover and persistence, Puppet Server authorization/mTLS/CA/JRuby/routes, r10k/Code Manager deployment, Bolt project/inventory/plans/tasks, and bounded Ruby module extensions for custom facts, modern/legacy functions, types, providers, and report processors; surfaces commands, dynamic/process execution, destructive changes, network access, privilege, filesystem/system mutation, unsafe deserialization, sensitive input, and controller/agent runtime boundaries without execution |
