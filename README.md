...
| Ansible project / inventory / Controller export / content policy / execution environment / Molecule | `readtheplan ansible-project controller-export.json` | Automation Controller/AWX import-export assets, RBAC, credentials, projects, inventory sources, job/workflow templates, schedules, notifications and execution environments; plus controller transport, plugins/callbacks, Galaxy dependencies, inventory, collection/role metadata, Ansible-lint policy, Builder/Navigator isolation, and Molecule providers/sequences/verifiers, with credential, provenance, cleanup, and execution boundaries |
...
| Chef project / dependencies / runtime | `readtheplan chef-project .kitchen.yml` | Policyfiles/locks, Berksfiles and dependency graphs, cookbook metadata, Infra Client, Workstation/knife, Solo, Server, and Test Kitchen configuration; migration, trust, credentials, provenance, graph integrity, execution, lifecycle hooks, drivers/transports, privilege, bootstrap, TLS/LDAP/database controls, and Ruby/ERB boundaries |
...
| Puppet project / Server policy / runtime / deployment / Bolt | `readtheplan puppet-project auth.conf` | Forge/Git modules, metadata, Hiera, agent settings, per-environment code/cache policy, PuppetDB failover and persistence, Puppet Server authorization/mTLS/CA/JRuby/routes, r10k/Code Manager deployment, and Bolt plugins/discovery/transports/privilege, with strict HOCON and non-execution boundaries |
