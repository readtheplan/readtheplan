...
| Puppet project / Server policy / runtime / deployment / Bolt | `readtheplan puppet-project modules/site/tasks/deploy.sh` | Forge/Git modules, metadata, Hiera, agent settings, per-environment code/cache policy, PuppetDB failover and persistence, Puppet Server authorization/mTLS/CA/JRuby/routes, r10k/Code Manager deployment, and Bolt project/inventory, YAML plans, task metadata, and bounded shell/PowerShell/Python/Ruby task implementations; commands, dynamic/process execution, destructive changes, network access, privilege, filesystem/system mutation, unsafe deserialization, sensitive input, no-op/remote execution, and runtime boundaries |
