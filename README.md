...
|------|---------|----------------|
| Project auto-discovery | `readtheplan scan .` | Recursively discovers high-confidence infrastructure inputs by canonical path or distinctive content, routes specialized formats such as Terraform/OpenTofu state, Crossplane, SAM, Serverless Framework, JCasC, Pulumi, Azure What-If, and Carvel to their native analyzers, skips dependency/build directories and symlinks, and returns one deterministic aggregate gate with per-file results and validation errors |
