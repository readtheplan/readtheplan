...
def agent_gate_to_dict(
    ...
    """Build the deterministic local agent gate contract for a Terraform plan."""
    ...
    reason = _reason(summary, decision, risk, counts)
    ...
    return {
        ...
        "evidence_checklist": _evidence_checklist(summary, decision, catalog),
        "auditor_summary": _auditor_summary(summary, decision, risk, counts),
        "risk_counts": counts,
    }
...
def _reason(
    ...
    if not summary.resource_changes:
        return "No Terraform resource changes were found; the agent may continue."
    if decision == "block":
        flagged = counts["dangerous"] + counts["irreversible"]
        return (
            f"Block because {flagged} Terraform change(s) are dangerous or "
            f"irreversible; human approval, change evidence, and security review "
            f"are required before merge or apply."
        )
    if decision == "warn":
        return (
            f"Warn because the highest Terraform risk tier is {risk}; reviewer "
            f"approval and change evidence are required before merge or apply."
        )
    return "Proceed because all Terraform resource changes are safe-tier."
...
def _auditor_summary(
    ...
    return (
        f"readtheplan evaluated {len(summary.resource_changes)} Terraform resource "
