...
def agent_gate_helmfile(
    input_path: str,
    framework: str | None = None,
) -> dict[str, object]:
    """Return the gate decision for local Helmfile state or lock data."""
    ...
    try:
        ...
        raise MCPToolInputError(
            code="INPUT_ERROR", message=f"Cannot read Helmfile input {input_path}: {exc}"
        ) from exc
    try:
        data = parse_helmfile(source, Path(input_path).name)
    except HelmfileInputError as exc:
        raise MCPToolInputError(
            code="INVALID_INPUT",
            message=f"Invalid Helmfile input in {input_path}: {exc}",
        ) from exc
    if not HelmfileAdapter().can_handle(data):
        raise MCPToolInputError(
            code="INVALID_INPUT",
            message="Input is not recognized as Helmfile configuration",
        )
    catalog = _load_catalog_for_tool(framework)
...
def agent_gate_carvel(
    input_path: str,
    framework: str | None = None,
) -> dict[str, object]:
    """Return the gate decision for a local Carvel-family artifact."""
    ...
    try:
        ...
        raise MCPToolInputError(
            code="INPUT_ERROR", message=f"Cannot read Carvel input {input_path}: {exc}"
        ) from exc
    try:
        data = parse_carvel(source, Path(input_path).name)
    except CarvelInputError as exc:
        raise MCPToolInputError(
            code="INVALID_INPUT",
            message=f"Invalid Carvel input in {input_path}: {exc}",
        ) from exc
    if not CarvelAdapter().can_handle(data):
        raise MCPToolInputError(
            code="INVALID_INPUT",
            message="Input is not recognized as Carvel configuration",
        )
    catalog = _load_catalog_for_tool(framework)
...
def agent_gate_sops(
    ...
    """Return the gate decision for local SOPS policy or encrypted data."""
    ...
    try:
        ...
        raise MCPToolInputError(
            code="INPUT_ERROR", message=f"Cannot read SOPS input {input_path}: {exc}"
        ) from exc
    try:
        data = parse_sops(source, Path(input_path).name)
    except SOPSInputError as exc:
        raise MCPToolInputError(
            code="INVALID_INPUT",
            message=f"Invalid SOPS input in {input_path}: {exc}",
        ) from exc
    if not SOPSAdapter().can_handle(data):
        raise MCPToolInputError(
            code="INVALID_INPUT",
            message="Input is not recognized as SOPS policy or encrypted data",
        )
    catalog = _load_catalog_for_tool(framework)
    return analyze_sops(data, catalog=catalog)
...
def create_server() -> Any:
    ...
    agent_gate_sops_handler = agent_gate_sops
    ...
    def _agent_gate_pipeline_tool(
        ...
        """Return a gate for supported CI pipeline YAML.
    ...
    @mcp.tool(name="agent_gate_helmfile")
    def _agent_gate_helmfile_tool(
        input_path: str,
        framework: str | None = None,
    ) -> dict[str, object]:
        """Return a gate for Helmfile state, Go-template state, or lock data.

        Args:
            input_path: Local path to Helmfile state or lock data.
            framework: Optional compliance framework for control checks.
        """
    ...
    @mcp.tool(name="agent_gate_carvel")
    def _agent_gate_carvel_tool(
        input_path: str,
        framework: str | None = None,
    ) -> dict[str, object]:
        """Return a gate for ytt, vendir, kbld, imgpkg, or kapp input.

        Args:
            input_path: Local path to a Carvel-family source or lock file.
            framework: Optional compliance framework for control checks.
        """
    ...
    @mcp.tool(name="agent_gate_sops")
    def _agent_gate_sops_tool(
        input_path: str,
        framework: str | None = None,
    ) -> dict[str, object]:
        """Return a gate for SOPS policy or encrypted data without decrypting it.

        Args:
            input_path: Local path to .sops.yaml or encrypted YAML/JSON/dotenv/INI data.
            framework: Optional compliance framework for control checks.
        """
        return agent_gate_sops_handler(input_path, framework=framework)
