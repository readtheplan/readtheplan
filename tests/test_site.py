...
def _legacy_site_has_client_onboarding_surface() -> None:
    ...
    assert "PILOT_HANDOFF_EMAIL" not in app
    ...
    assert "pilot handoff" not in app.lower()
...
def test_site_ui_handoff_contracts() -> None:
    ...
    terms = (SITE / "terms" / "index.html").read_text(encoding="utf-8")
    chat = (SITE / "chat" / "index.html").read_text(encoding="utf-8")
    ...
    assert "Managed (Paid)" not in terms
    assert "Enterprise (Custom)" not in terms
    assert "no paid feature tier" in terms
    assert "AI Project Guide" in chat
    assert "sales agent" not in chat.lower()
...
def test_static_seo_tools_preserve_local_first_privacy() -> None:
    ...
    assert "Open the free quickstart" in combined
    assert "Request pilot setup" not in combined
    assert "mailto:info@readtheplan.dev" not in combined
